...
module.exports = (function() {
  ...
  var _memStore = {},
      _file = '.fhc_local/db.json',
      _ready = false;

  // Set up the event listeners to persist our data.
  ...
  function list(options, cb) {
    ...
    // Support for the `eq` filter.
    if (options.eq && typeof options.eq === 'object') {
      results = results.filter(function(result) {
        var include = false,
            key;
        for (key in options.eq) {
          if (result.fields[key] && result.fields[key] === options.eq[key]) {
            include = true;
          }
        }
        return include;
